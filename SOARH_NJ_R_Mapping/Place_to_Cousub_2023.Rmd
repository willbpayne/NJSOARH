...
state_name <- 'AL Alabama' # set this once and everything flows from here
...
write.csv(unjoinable_units_by_type, paste0(state_abbreviation,"_unjoinable_units_by_type_2022.csv",sep=''), row.names=FALSE) # writes filename with state abbreviation, update later for different years
...
state_code <- '34' # use this for testing
...
CDP_2010_Neg_Buffer <- CDP_2010_Proj %>% st_buffer(-150)  # create the negative 1000 foot buffer
CDP_2010_Neg_Buffer$sqkm <- as.numeric(st_area(CDP_2010_Neg_Buffer)/1000000)
...
CDP_Cousub_intersects <- st_intersection(CDP_2010_Neg_Buffer, Cousubs_2010_Proj) # try this
...
CDPs_with_Munis_Duplicates_Total <- duplicated(pull(CDP_Cousub_intersects,PLACEFP10)) | duplicated(pull(CDP_Cousub_intersects,PLACEFP10), fromLast=TRUE) # get duplicates and originals
CDPs_with_Munis_Duplicates_Total_List <- CDP_Cousub_intersects[CDPs_with_Munis_Duplicates_Total,c("NAMELSAD10", "NAMELSAD10.1","coverage")]
...
place_PSH_CDPs_with_Munis <- left_join(place_PSH_CDPs, CDP_Correspondence_Table, by = "code") # put them together
...
# come up with a list of cousubs that need to be manually aggregated for 2022 (NOT UPDATED)
