...
nj_tracts <- st_read("~/Box/NJSOARH/Data/Spatial_Data/Reference_Layers/nhgis_nj_tracts_2020.geojson")
...
nj_tracts_PSH <- left_join(nj_tracts, PictureSubsidizedHousing, by = c("GEOID" = "geoid")) # put them together

#tm_shape(osm_NJ) + tm_rgb() + 
tmap_mode("view")

tm <- tm_shape(nj_tracts_PSH) +
    tm_polygons(c("share_tot","share_rental"), style = "jenks", legend.hist = TRUE) + tm_facets(sync = TRUE, nrow = 1) + tm_layout(legend.outside = FALSE)
