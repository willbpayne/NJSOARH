...
place_PSH_Final <- place_PSH_noNAs %>%
  ...
  summarise(count = n(), 
            ...
            total_pct_occupied = (sum(Pg1_pct_occupied * Pg1_total_units, na.rm=TRUE))/sum(Pg1_total_units, na.rm=TRUE),
            # Program 2 - Public Housing (72 PLACE entries with units, 65 reporting demographics) #
            pg2_units = sum(Pg2_total_units, na.rm=TRUE), 
            pg2_reported = sum(Pg2_number_reported, na.rm=TRUE), 
            pg2_pct_occupied = (sum(Pg2_pct_occupied * Pg2_total_units, na.rm=TRUE))/sum(Pg2_total_units, na.rm=TRUE),
            ...
            pg2_months_since_report = (sum(pg2_units * Pg2_months_since_report, na.rm=TRUE))/sum(pg2_units, na.rm=TRUE),
            pg2_people_total = sum(Pg2_people_total, na.rm=TRUE),
            pg2_people_per_unit = (sum(Pg2_people_total, na.rm=TRUE)/sum(pg2_occupied_units, na.rm=TRUE)),
...
place_PSH_Final <- filter(place_PSH_Final, !is.na(county)) # get rid of duplicate rows
