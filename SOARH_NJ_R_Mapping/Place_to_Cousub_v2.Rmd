...
place_PSH <- read.csv(file = 'NJ_Place_2021.csv')

place_PSH <- subset(place_PSH, select = -c(X, gsl,states,sumlevel,sub_program,entities, fedhse, cbsa, latitude, longitude, state, pha_total_units, ha_size))
...
# come up with a list of cousubs that need to be manually aggregated for 2021

manual_tract_aggregation_list <- c(3402970320,3402903050,3401304695,3401306260,3400129280,3402129310,3401133120,3402133180,3401139450,3402139510,3401547250,3402347280,3402554270,3402954300,3401577180,3402777240,3404177300,3401974420,3400569990,3403970020,3401924870,3401524840,3403974480,3400577150,3400377135)
...
#Springfield (Union) 3403970020

#Washington (Burlington) 3400577150
#Washington (Bergen) 3400377135
...
#Franklin (Hunterdon) 3401924870
#Franklin (Gloucester) 3401524840
#Union (Hunterdon) 3401974420
#Union (Union) 3403974480
...
place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ FRANKLIN TOWNSHIP"] <- 3401524840 # manually set Franklin Township (Gloucester County)
...
place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ UNION TOWNSHIP"] <- 3403974480 # manually set Union Township (Union County) 
...
PSH_trct_2021<-read.csv("NJ_TRACT_2021.csv")

#will's code part 1
PSH_trct_2021[PSH_trct_2021==-1]<-0 # replace -1 with 0 throughout (missing data)
PSH_trct_2021[PSH_trct_2021==-4]<-NA # replace -4 with NA throughout (suppressed demographic data)
PSH_trct_2021[PSH_trct_2021==-5]<-NA # replace -5 with NA throughout (reporting under 50% demographic data)
prefixes <- unique(PSH_trct_2021$program) # get all the unique program names used in the dataset
PSH_trct_2021 <- pivot_wider( # pivot them to one row per cdp/cousub/remainder
  PSH_trct_2021,
  id_cols = c(code),
  names_from = program,
  names_prefix = "",
...
# sort the dataframe so that each set of unit counts and demographic variables are together by program
names_to_order <- map(prefixes, ~ names(PSH_trct_2021)[grep(paste0(.x,"_"), names(PSH_trct_2021))]) %>% unlist
names_id <- setdiff(names(PSH_trct_2021), names_to_order)
PSH_trct_2021 <- PSH_trct_2021 %>%
  select(names_id, names_to_order)
rm(names_id, names_to_order,prefixes) # remove intermediate outputs

# sum occupied units for the programs that have any reporting (not 6 or 7) to make formulas easier later
PSH_trct_2021$pg1_occupied_units <- PSH_trct_2021$Pg1_total_units * PSH_trct_2021$Pg1_pct_occupied / 100
PSH_trct_2021$pg2_occupied_units <- PSH_trct_2021$Pg2_total_units * PSH_trct_2021$Pg2_pct_occupied / 100
PSH_trct_2021$pg3_occupied_units <- PSH_trct_2021$Pg3_total_units * PSH_trct_2021$Pg3_pct_occupied / 100
PSH_trct_2021$pg4_occupied_units <- PSH_trct_2021$Pg4_total_units * PSH_trct_2021$Pg4_pct_occupied / 100
PSH_trct_2021$pg5_occupied_units <- PSH_trct_2021$Pg5_total_units * PSH_trct_2021$Pg5_pct_occupied / 100
PSH_trct_2021$pg8_occupied_units <- PSH_trct_2021$Pg8_total_units * PSH_trct_2021$Pg8_pct_occupied / 100
PSH_trct_2021$pg9_occupied_units <- PSH_trct_2021$Pg9_total_units * PSH_trct_2021$Pg9_pct_occupied / 100

###my code

#Using LODES crosswalk

NJ_xwalk=gzfile('nj_xwalk.csv.gz','rt') 
...
#append xwalk
PSH_trct_appended <- left_join(PSH_trct_2021, NJ_xwalk_trct, by=c("code"="trct"))

#select ctysubs of interest

PSH_trct_appended <- subset(PSH_trct_appended, ctycsub=='3402970320'|ctycsub=='3402903050'|ctycsub=='3401304695'                  |ctycsub=='3401306260'|ctycsub=='3401974420'|ctycsub=='3400569990'|ctycsub=='3400129280'    |ctycsub=='3402129310'|ctycsub=='3401133120'|ctycsub=='3402133180'|ctycsub=='3401139450'|ctycsub=='3402139510'|ctycsub=='3401547250'|ctycsub=='3402347280'|ctycsub=='3402554270'|ctycsub=='3402954300'|ctycsub=='3401577180'|ctycsub=='3402777240'|ctycsub=='3404177300'|ctycsub=='3400569990'|ctycsub=='3403970020'|ctycsub=='3400577150'|ctycsub=='3400377135'|ctycsub=='3401924870'|ctycsub=='3401524840'|ctycsub=='3401974420'|ctycsub=='3403974480')
##will's code part 2
...
place_PSH_trcts_Final$county <- str_extract_all(place_PSH_trcts_Final$county, "\\([^()]+\\)")
...
place_PSH_trcts_Final$name2 <- strsplit(place_PSH_trcts_Final$name,'\\(')[[1]]

place_PSH_trcts_Final <- separate(place_PSH_trcts_Final, col = name, into = c("name","county"), sep = "\\(")


# replace all the infinite and NaN values with NA to avoid causing trouble with mapping?
write.csv(place_PSH_trcts_Final, 'place_2021_PSH_trcts_Final.csv', na = "")
