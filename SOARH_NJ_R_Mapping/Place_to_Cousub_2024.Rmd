...
place_PSH_2023 <- read.csv(file = 'PLACE_2023.csv') # read in national place-level data
...
place_PSH <- place_PSH_2023 %>% filter(grepl(state_name, states)) # filter to single state
...
if (state_code == '09'){
    place_PSH_CDPs <- place_PSH %>% filter(grepl('CDP|BOROUGH', name)) # extract CDPs
    print("I'm in Connecticut!")
  } else {
    if (state_code == '36'){
      place_PSH_CDPs <- place_PSH %>% filter(grepl('CDP|VILLAGE', name)) # extract CDPs
...
place_PSH_geoID <- place_PSH_geoID %>% filter(!grepl('REMAINDER', name)) # remove fake remainders from here
...
CDP_2020_Neg_Buffer$sqkm <- as.numeric(st_area(CDP_2020_Neg_Buffer)/1000000)

Census_Cousubs_2020 <- st_read(paste("Census_Geographies/2020_cousub/tl_2020_", state_code,"_cousub/tl_2020_", state_code, "_cousub.shp", sep = "")) # bring in Census Place level geometries
Census_Cousubs_2020 <- subset(Census_Cousubs_2020, select = -c(CNECTAFP,NECTAFP,NCTADVFP,STATEFP,INTPTLAT,INTPTLON,ALAND,AWATER,FUNCSTAT,LSAD,MTFCC))
Cousubs_2020_Proj <- Census_Cousubs_2020 %>% sf::st_transform(paste0("+proj=utm +zone=",utm_zone_simple," +ellps=GRS80 +datum=NAD83", sep="")) # reproject to UTM projection specified above

CDP_Correspondence_Table <- st_join(CDP_2020_Neg_Buffer, Cousubs_2020_Proj, suffix = c("_CDP", "_Cousub"))
CDP_Correspondence_Table <- subset(CDP_Correspondence_Table, select = -c(CLASSFP)) # remove extra field that we might use elsewhere but not now

CDP_Cousub_intersects <- st_intersection(CDP_2020_Neg_Buffer, Cousubs_2020_Proj) # try this
...
## ADD CODE TO CHECK PROJECT LEVEL AND TRACT LEVEL PSH TO DISAMBIGUATE
## ANYTHING AT THE PROJECT LEVEL WE CAN RIGOROUSLY ASSIGN TO A COUSUB
## HCV UNITS WE NEED TO PROPORTIONALLY ASSIGN AND/OR BUILD UP FROM TRACTS
...
CDPs_with_Munis_Duplicates_Total
colnames(CDPs_with_Munis_Duplicates_Total)
...
Ambiguous_CDP_Muni_List <- CDPs_with_Munis_Duplicates_Total_List[['GEOID.1']]
Ambiguous_CDP_Muni_List_Deduplicated <- unique(Ambiguous_CDP_Muni_List)

Ambiguous_Muni_Name_List_Character <- as.character(Ambiguous_Muni_Name_List)

Tract_Aggregation_List <- unique(c(Ambiguous_CDP_Muni_List_Deduplicated, Ambiguous_Muni_Name_List_Character))

CDPs_with_Munis_Duplicates_Total_No_Geo <- st_drop_geometry(CDPs_with_Munis_Duplicates_Total_List) # remove geometry

# JOIN THIS BY CODE TO PSH

CDPs_with_Munis_Duplicates_Total_Export <- left_join(CDPs_with_Munis_Duplicates_Total_No_Geo, place_PSH_CDPs, by = c("GEOID" = "code")) # put them together, but this is getting too much

write.csv(CDPs_with_Munis_Duplicates_Total_Export, 'place_2023_CDP_duplicates.csv', na = "")
...
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table, !(GEOID_Cousub == 3403709160 & NAMELSAD_CDP == 'Lake Mohawk CDP')) # remove one of the ambiguous CDPs since all housing units are in Sparta Township, not Byram Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3400553070 & NAMELSAD_CDP == "McGuire AFB CDP")) # remove one of the ambiguous CDPs since all housing units are in North Hanover Township, not New Hanover Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3403524900 & NAMELSAD_CDP == "Kingston CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in South Brunswick Township, not Franklin Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3404106160 & NAMELSAD_CDP == "Hainesburg CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in Knowlton Township, not Blairstown Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3403770890 & NAMELSAD_CDP == "Crandon Lakes CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in Hampton Township, not Stillwater Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3400941610 & NAMELSAD_CDP == "Rio Grande CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in Middle Township, not Lower Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3401543440 & NAMELSAD_CDP == "Richwood CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in Harrison Township, not Mantua Township
CDP_Correspondence_Table_clean <- filter(CDP_Correspondence_Table_clean, !(GEOID_Cousub == 3404143320 & NAMELSAD_CDP == "Port Colden CDP")) # remove one of the ambiguous CDPs since HCV units are more likely to be in Washington Township, not Mansfield Township

CDP_Correspondence_Table_no_geo <- st_drop_geometry(CDP_Correspondence_Table_clean) # remove geometry

place_PSH_CDPs_with_Munis <- left_join(place_PSH_CDPs, CDP_Correspondence_Table_no_geo, by = c("code" = "GEOID_CDP")) # put them together

write.csv(place_PSH_CDPs_with_Munis, 'place_PSH_CDPs_with_Munis_2023_no_geo.csv', na = "") # write a file to check in order to make the table
...
CDPs_with_Ambiguous_or_Missing_Munis <- place_PSH_CDPs_with_Munis[CDPs_with_Munis_Duplicates, c('name','code',"NAMELSAD_Cousub")]
CDPs_with_Ambiguous_or_Missing_Munis <- CDPs_with_Ambiguous_or_Missing_Munis %>% filter(!grepl('REMAINDER', name)) # remove fake remainders from here

#subset(place_PSH_CDPs_with_Munis,duplicated(name))

place_PSH_CDPs_Manual <- filter(place_PSH_CDPs_with_Munis, is.na(NAMELSAD_Cousub))
...
# come up with a list of cousubs that need to be manually aggregated for NJ in 2023
manual_tract_aggregation_list <- c(3402970320, # Stafford (Ocean) - because of Ocean Acres CDP
                                   3402903050, # Barnegat (Ocean) - because of Ocean Acres CDP
                                   3401304695, # Belleville (Essex) - because of Silver Lake CDP
                                   3401306260, # Bloomfield (Essex) - because of Silver Lake CDP
                                   3403531890, # Hillsborough (Somerset) - because of Belle Mead CDP
                                   3403547580, # Montgomery (Somerset) - because of Belle Mead CDP
                                   3400726760, # Gloucester (Camden) - because of Sicklerville CDP
                                   3400781740, # Winslow (Camden) - because of Grenloch and Sicklerville CDPs
                                   3401577180, # Washington (Gloucester) - because of Grenloch CDP
                                   
                                   # END AMBIGUOUS CDPs, START AMBIGUOUS REMAINDERS AND TOWNSHIPS
                                   3400129280, # Hamilton (Atlantic) 
                                   3402129310, # Hamilton (Mercer)
                                   3401133120, # Hopewell (Cumberland)
                                   3402133180, # Hopewell (Mercer)
                                   3401139450, # Lawrence (Cumberland)
                                   3402139510, # Lawrence (Mercer)
                                   #3401547250, MONROE
                                   #3402347280, MONROE
                                   # MANSFIELD
                                   # MANSFIELD
                                   #3402554270, OCEAN
                                   #3402954300, OCEAN
                                   3401577180, # Washington Township (Gloucester)
                                   3402777240, # Washington Township (Morris)
                                   3404177300, # Washington Township (Warren)
                                   3400569990, # Springfield Township (Burlington)
                                   3403970020, # Springfield Township (Union)
                                   3401924870, # Franklin Township (Hunterdon)
                                   3401524840, # Franklin Township (Gloucester)
                                   3403974480, # Union Township (Union)
                                   3401974420, # Union Township (Hunterdon)
                                   3400577150, # Washington Township (Burlington)
                                   3400377135) # Washington Township (Bergen)
...
NJ_Munis_for_Matching_Duplicates <- duplicated(pull(NJ_Munis_for_Matching,MUN_LABEL)) | duplicated(pull(NJ_Munis_for_Matching,MUN_LABEL), fromLast=TRUE)
Ambiguous_Muni_Names <- NJ_Munis_for_Matching[NJ_Munis_for_Matching_Duplicates, c('COUNTY','MUN_LABEL',"CENSUS2010")]
Ambiguous_Muni_Name_List <- Ambiguous_Muni_Names[['CENSUS2010']]
...
# FAIRFIELD
# Fairfield Township (Essex) is 3401322385
# Remainder of Fairfield Township (Cumberland) is 3401122350

# FRANKLIN
# 3403524900 (Somerset) is entirely made of CDPs in both years, so doesn't need handling
# 3401924870 (Hunterdon) has no CDPs in both years, so could be Franklin Township
# 3401524840 (Gloucester) has CDPs but didn't in 2010, so could be Franklin Township
# 3404124960 (Warren) has CDPs in both years, so is Remainder of Franklin Township

#3425050 FRANKLIN TOWNSHIP in Gloucester is actually a new CDP, AAAAAAAH - but only matters here and is solved by tract aggregation
#3442840 FRANKLIN TOWNSHIP in Gloucester is actually a new CDP, AAAAAAAH - but only matters here and is solved by tract aggregation

# Wellllll, there could be other unsuccessful joins by muni code, need to check that since things that say township should be easy joins but won't be if they're actually new CDPs; are we joining these on code or on name alone?

# GREENWICH
# 3401128170 (Cumberland) has no CDPs in 2010, yes in 2020, is Township
# 3401528185 (Gloucester) has CDPs in both years, could be Remainder (CHECK TRACTS?)
# 3404128260 (Warren) has CDPs in both years, could be Remainder

# HAMILTON
# 3400129280 (Atlantic), has CDPs both years, could be Remainder 
# 3402129310 (Mercer), has CDPs both years, could be Remainder

# HOPEWELL
# 3401133120 (Cumberland) only has CDPs in 2020, could be Township
# 3402133180 (Mercer) only has CDPs in 2020, could be Township

# LAWRENCE
# 3401139450 (Cumberland) has CDPs both years, could be Remainder
# 3402139510 (Mercer) has CDPs both years, could be Remainder
...
# SPRINGFIELD
# Remainder of Springfield Township is 3403970020 (Union) or 3400569990 (Burlington) TRACTS SO AMBIGUOUS ONLY ONE UNIT THOUGH

# UNION
# Union Township could be 3401974420 (Hunterdon, no CDPs either year) or 3403974480 (Union, no CDPs in 2010, yes in 2020) DOESNT WORK BECAUSE OF WEIRD 2010-2020 CDP CHANGES

# WASHINGTON (all need to be built from tracts)
# 3404177300 (Warren) has CDPs both years, could be Remainder 
# 3402777240 (Morris) has CDPs both years, could be Remainder
# 3401577180 (Gloucester) has CDPs both years, could be Remainder
# 3400577150 (Burlington) has no CDPs both years, could be Township
# 3400377135 (Bergen) has no CDPs both years, could be Township
...
Cousubs_2020_Proj_Clean <- subset(Cousubs_2020_Proj, select = -c(CLASSFP,COUNTYFP,COUSUBFP,COUSUBNS))

Tract_Cousubs_2020 <- subset(Cousubs_2020_Proj_Clean, GEOID %in% Tract_Aggregation_List)

########

place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ CITY OF ORANGE TOWNSHIP"] <- 3401313045 # manually set City of Orange Township (Essex County)
place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ PRINCETON TOWNSHIP"] <- 3402160900 # manually set Princeton Township (Mercer County) to Princeton Borough (Mercer County) since they've merged

place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ GREENWICH TOWNSHIP"] <- 3401128170 # manually set Greenwich Township (Cumberland County)
place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ SPRINGFIELD TOWNSHIP"] <- 3403970020 # manually set Springfield Township (Union County)
place_PSH_Unmerged_Final$CENSUS2010[place_PSH_Unmerged_Final$name == "NJ WASHINGTON TOWNSHIP"] <- 3400377135 # manually set Washington Township (Bergen County) 
