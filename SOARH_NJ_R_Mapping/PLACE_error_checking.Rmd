...
## THIS STEP DOES OUR CDP TO COUSUB MAPPING FOR ANY STATE
...
Census_CDPs_2010_Neg_Buffer_500 <-  Census_CDPs_2010 %>% sf::st_transform(3424) %>% # reproject the data to NJ state plane
    sf::st_buffer(-1000) # create the negative 1000 foot buffer
...
Census_Cousubs_2010 <- Census_Cousubs_2010 %>% sf::st_transform(3424) # reproject to NJ state plane

CDP_Correspondence_Table <- st_join(Census_CDPs_2010_Neg_Buffer_500, Census_Cousubs_2010, suffix = c("_CDP", "_Cousub"))
CDP_Correspondence_Table <- st_drop_geometry(CDP_Correspondence_Table)
CDP_Correspondence_Table <- subset(CDP_Correspondence_Table, select = -c(CLASSFP10))

CDPs_with_Munis_Duplicates_Total <- duplicated(pull(CDP_Correspondence_Table,NAMELSAD10_CDP)) | duplicated(pull(CDP_Correspondence_Table,NAMELSAD10_CDP), fromLast=TRUE)
